...
def test_runner_only_fires_each_businesss_rules_against_its_own_customers(isolated_db, monkeypatch):
    ...
    # High lead_score condition both customers will match, one rule per
    # business, via the API-layer manager.create_rule (the path the
    # dashboard's rule editor actually uses).
    ...
    assert performance_b[0]["customers_matched"] == 1
